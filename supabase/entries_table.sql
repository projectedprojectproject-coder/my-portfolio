...
('publications', 0, $t$출간$t$, $t$AI와 할리우드 공식으로 만드는 방구석 시네마틱$t$, $t$이윤호 지음$t$,
  ...
  {"type":"text","text":"그런데 여기서 중요한 질문이 하나 남는다. 같은 AI와 같은 툴을 썼는데 왜 어떤 영상은 수백만 명을 사로잡고 어떤 영상은 아무 반응도 얻지 못하는가. 이 책의 답은 단순하다. 사람을 움직이는 것은 기술이 아니라 이야기이기 때문이다. 할리우드는 100년이 넘는 시간 동안 수많은 성공과 실패를 반복하며 관객의 감정을 움직이는 서사 구조를 검증해 왔다. 이 책은 바로 그 지점에서 시작한다. AI 툴 사용법을 나열하는 매뉴얼이 아니라, 할리우드가 검증한 스토리 공식과 AI 제작 기술을 하나의 작업 흐름으로 연결하는 책이다."},
  {"type":"text","text":"전반부에서는 인간이 이야기에 몰입하는 이유에서 출발해, 세이브 더 캣의 핵심 원리, 주인공의 Want와 Need, 3막 구조와 15비트 시트까지 할리우드 스토리 설계법을 단계별로 다룬다. 후반부에서는 그 설계를 실제 영상으로 옮기는 과정을 따라간다. AI와 함께 기획하고 대본을 쓰는 법, 스토리보드 설계, 이미지 생성, 비디오 생성, AI 편집과 후반 제작, 그리고 숏폼 콘텐츠 실전 제작까지 기획에서 배포에 이르는 전 과정을 순서대로 안내한다."},
  {"type":"text","text":"이 책의 가장 큰 특징은 하나의 작품이 책 전체를 관통한다는 점이다. 인류가 떠난 지구에 홀로 남겨진 소녀 줄리아와, 감정을 이해하지 못하는 거대 전투 로봇 RTLX0329의 여정을 그린 SF 단편 UNCANNY VALLEY다. 이 작품이 로그라인에서 15비트 구조, 캐릭터 디자인, 콘셉트 아트, 스토리보드, 영상 생성까지 각 장의 이론이 실제로 어떻게 적용되는지를 보여 주는 실습 사례로 이어진다. 독자는 이론을 배우는 동시에 한 편의 작품이 완성되어 가는 과정을 처음부터 끝까지 지켜보게 된다."},
  {"type":"text","text":"부록도 실전 중심으로 구성했다. 기획부터 편집까지 바로 쓸 수 있는 AI 프롬프트 100선, 자신의 이야기를 직접 설계해 보는 제작 워크북, AI 창작 생태계의 여섯 단계 지형도, UNCANNY VALLEY 스토리 피치 전문, 그리고 단계별 실전 툴 가이드를 담았다. 유튜브 쇼츠 하나를 만들고 싶은 사람부터 단편영화를 꿈꾸는 사람까지, 머릿속에만 있던 이야기를 세상 밖으로 꺼내고 싶은 모든 창작자를 위한 책이다. 기술은 계속 바뀌지만 좋은 이야기의 원리는 변하지 않는다. 아이디어가 막힐 때, AI가 최고의 공동 작가가 되어 줄 것이다."}
]$j$::jsonb);
